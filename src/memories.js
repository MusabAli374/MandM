...
function lilyBase(folder) {
  return `${BASE}models/${folder}`;
}
...
export const memories = Object.entries(themes).map(([folder, theme]) => ({
  ...theme,
  folder,
  lilyBase: lilyBase(folder),
  photos: photoFiles[folder].map(filename => photo(folder, filename))
}));